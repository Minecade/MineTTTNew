package com.minecade.minettt.enderbar;
...
import org.orion304.enderbar.FakeDragon;
import org.orion304.enderbar.Util;
...
public class BarAPI extends JavaPlugin implements Listener {
	...
	public static void setMessage(final Player player, String message,
		...
		timers.put(
				player.getName(),
				Bukkit.getScheduler()
						.runTaskTimer(MineTTT.getPlugin(),
								new BukkitRunnable() {
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(MineTTT.getPlugin(), new Runnable() {
